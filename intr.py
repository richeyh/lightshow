...
p_forby = re.compile("for ([A-Za-z_]+) from ([A-Za-z0-9_.\[\]]+) to ([A-Za-z0-9_.\[\]]+) by ([0-9]+)")
p_for = re.compile("for ([A-Za-z_]+) from ([A-Za-z0-9_.\[\]]+) to ([A-Za-z0-9_.\[\]]+):")
p_if = re.compile("if (.*):")
...
patterns = [p_forby, p_for, p_if, p_elif, p_else, p_define, p_assign, p_func, p_incdec, p_operate]
patternNames = ["ForBy", "For", "If", "Elif", "Else", "Define", "Assign", "Func", "IncDec", "Operate"]
...
def generateCode(lines):
	...
	for line in lines:
		...
		if line["type"] in ["For", "ForBy"]:
			...
			codeLine += var + " " + compare + " " + upper + "; " + var + " += " + by + "){"
		elif line["type"] == "If":
			...
			if func == "setPixel":
				...
				if len(arguments) == 2:
					cvar = arguments[1]
					arguments = [arguments[0], cvar + ".r", cvar + ".g", cvar + ".b"]
			elif func == "show":
				func = "leds.show"
			elif func == "delay":
				arguments = ["DELAY"]
